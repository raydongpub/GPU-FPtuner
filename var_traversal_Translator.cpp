...
class fpInstrument{
  public:
    ...
    void tuning_visit(SgSourceFile* sfile, int cur_iter, int tune_op);
  ...
  for (Rose_STL_Container<SgNode*>::iterator i = nodeList.begin(); 
    ...
    switch (stmt->variantT())
    {
      ...
      case V_SgForStatement:
        ...
        // insert counter variable for dynamic loop profiling
        string out_stream = "loop" + Rose::StringUtility::numberToString(loop_id); 
        SgVariableDeclaration* out_var = buildVariableDeclaration (out_stream, buildOpaqueType("ofstream", getScope(*(i-1))));
        insertStatementBefore(stmt, out_var, false);
        string stream_open = "loop" + Rose::StringUtility::numberToString(loop_id) + ".open(\"loop.txt\");";
        SgStatement* stmopen_stmt = buildStatementFromString(stream_open, getScope(*i));
        insertStatementBefore(stmt, stmopen_stmt, false);
        string prt_cnt = "printf(\"" + loop_var + " %d\\n\"," + loop_var + ");";
        //string prt_cnt = "std::cout<< \"" + loop_var + " \"<< " + loop_var + "<< std::endl;";
        //string prt_cnt = "fopen(\"loopinfo\", \"w\");";
        SgStatement* prt_stmt = buildStatementFromString(prt_cnt, getScope(*(i-1)));
        SgStatement* if_cond = buildStatementFromString("1>0", getScope(*i));
        SgStatement* if_stmt = buildIfStmt(if_cond, prt_stmt, stmt);
        insertStatementAfter(stmt, if_stmt, false);
        #endif
...
void fpInstrument::tuning_visit(SgSourceFile* sfile, int cur_iter, int tune_op) {
...
void fpInstrument::function_visit(SgSourceFile* sfile) {
  ...
  string f_line;
  Rose_STL_Container<string> func_visit;
  ifstream func_file("func_list.txt");
  ...
  ifstream var_file("var_list.txt");
  if (var_file.is_open()) {
...
class varsTraversal : public AstSimpleProcessing {
  public:
...
void varsTraversal::visit(SgNode* node) {
  ...
  SgVariableDeclaration* varsdeclaration = isSgVariableDeclaration(node);
  ...
  if (isSgFunctionDefinition(node) != NULL) {
    cur_func = isSgFunctionDefinition(node); 
    cur_func_name = cur_func->get_declaration()->get_name();  
    if (fpinstrument.func_map.find(cur_func_name) != fpinstrument.func_map.end()) {
      func_type = 2;
  ...
  // find floating-point variables and replace its type with high-precision types
  if (varsdeclaration != NULL) {
    //SgType* type =  getFirstVarType(varsdeclaration);
    //SgType* type =  varsdeclaration->get_baseTypeDefiningDeclaration()->get_type();
    SgType* type =  varsdeclaration->get_definition()->get_type();

    if (type->hasExplicitType()) {
      def_type = type->dereference();
      if (isSgTypedefType(def_type)) {
          cout << "\toriginal type: "     << node->unparseToString() << "func type: " << func_type << "\n";
        SgTypedefType* tydef = isSgTypedefType(def_type);
        if (tydef->get_name().getString() == "double3") {
          cout << "func: "     << cur_func_name.getString() << ", func_type: " << func_type <<"\n";
          // change floating-point variable into host or device high-precision
          //4: functions not visit, 1: main function, 2: device function, 3: global definition
          SgInitializedName* def_name = varsdeclaration->get_definition()->get_vardefn();
          cur_var_name = def_name->get_name().getString();
          SgInitializedName* init_name = varsdeclaration->get_decl_item(def_name->get_name());
          switch (func_type) {
          case 1:
            if (find(fpinstrument.var_device.begin(), fpinstrument.var_device.end(), init_name->get_name().getString()) != 
                fpinstrument.var_device.end()){
              cout << "\tdevice variable.\n";
          ...
          cout << "\tdouble3 type: "     << node->unparseToString() << "\n";
          cout << "\t definition: " << varsdeclaration->get_definition()->get_vardefn()->get_name().getString() << "\n";
        }
          cout << "\tother type: "     << node->unparseToString() << "func type: " << func_type << "\n";
          cout << "\tother definition: " << varsdeclaration->get_definition()->get_vardefn()->get_name().getString() << "\n";
      }
      //def_type = type->findBaseType();
      if (def_type->isFloatType()) {
        // Get the declaration statement information
        // note: intialized name is the variable name
        //SgInitializedName* init_name = varsdeclaration->get_definition()->get_vardefn();
        SgInitializedName* def_name = varsdeclaration->get_definition()->get_vardefn();
        cur_var_name = def_name->get_name().getString();
        SgInitializedName* init_name = varsdeclaration->get_decl_item(def_name->get_name());
        // change floating-point variable into host or device high-precision
        ...
        // set the floating-point declaration into desired high-precision
        cout << "\tdouble pointer type: "     << node->unparseToString() << "\n";
        cout << "\t definition: " << varsdeclaration->get_definition()->get_vardefn()->get_name().getString() << "\n";
      }
    } else if (type->isFloatType()) {
      //SgInitializedName* init_name = getFirstInitializedName(varsdeclaration);
      SgInitializedName* def_name = varsdeclaration->get_definition()->get_vardefn();
      cur_var_name = def_name->get_name().getString();
      SgInitializedName* init_name = varsdeclaration->get_decl_item(def_name->get_name());
      cout << "func: "     << cur_func_name.getString() << ", func_type: " << func_type <<"\n";
      ...
      cout << "\tdouble type: "     << node->unparseToString() << "\n";
      cout << "\tdefinition: " << varsdeclaration->get_definition()->get_vardefn()->get_name().getString() << "\n";
    }
    //printf("variable types= %p = %s \n", type, type->class_name().c_str());
  }
  // find the new instruction and replace floating-point allocation with high-precision types
  if (new_expr != NULL) {
    SgType* new_type = new_expr->get_type();
    if (new_type->hasExplicitType()) {
      if (new_type->findBaseType()->isFloatType()) {
        // replace the base type of floating-point allocation into high-precision
        //new_expr->get_specified_type()->reset_base_type(buildOpaqueType("dd_real", getScope(node)));
        //printf("\tnew types= %p = %s \n", new_type->findBaseType(), new_type->findBaseType()->class_name().c_str());
        cout << "func: "     << cur_func_name.getString() << ", func_type: " << func_type <<"\n";
        cout << "\tdouble new type: "     << node->unparseToString() << "\n";
        cout << "\tdefinition: " << cur_var_name << "\n";
      }
    }
  }
  // find the new instruction and replace floating-point allocation with high-precision types
  if (delete_expr != NULL) {
    SgType* delete_type = delete_expr->get_type();
    if (delete_type->hasExplicitType()) {
      if (delete_type->findBaseType()->isFloatType()) {
        // replace the base type of floating-point allocation into high-precision
        //new_expr->get_specified_type()->reset_base_type(buildOpaqueType("dd_real", getScope(node)));
        printf("\tdelete types= %p = %s \n", delete_type->findBaseType(), delete_type->findBaseType()->class_name().c_str());
        cout << "func: "     << cur_func_name.getString() << ", func_type: " << func_type <<"\n";
        cout << "\tdouble delete type: "     << node->unparseToString() << "\n";
      //cout << "\tdefinition: " << varsdeclaration->get_definition()->get_vardefn()->get_name().getString() << "\n";
      }
    }
  }
    ...
    if (file_list.size()>0) {
      ...
      for (iter=file_list.begin(); iter!=file_list.end(); iter++) {
        ...
        if (sfile != NULL) {
          ...
          if (t_iter > 0) {
            cout << "\t tuning start: \n";
            varsTraversal varsvisitor;
            varsvisitor.fpinstrument.function_visit(sfile);
            //fpinstrument.tuning_visit(sfile, t_iter, tune_op);
            //fpinstrument.print_fp_operation_count();
            // Merge the testing code from Virtual machine
            varsvisitor.traverseInputFiles(project,preorder);
