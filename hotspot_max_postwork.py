...
def max_pattern(var_list, host_vars, db_vars, fl_vars, sd_vars, h_new, n_size, n_ptr, stmt_line):
      ...
      if "double" in stmt_line or "dd_real" in stmt_line:
        if "=" in stmt_line and ("double" in stmt_line.split("=")[0] or "dd_real" in stmt_line) and "var" in stmt_line.split("=")[0]:
          if "dd_real" not in stmt_line:
            cur_lhs = stmt_line.split("=")[0].replace("double", "gdd_real")
          elif "gdd_real" not in stmt_line:
            cur_lhs = stmt_line.split("=")[0].replace("dd_real", "gdd_real")
          print(stmt_line.replace(stmt_line.split("=")[0], cur_lhs))
          ret = 11
        elif "=" not in stmt_line:
          if var in sd_vars and "__shared__" not in stmt_line:
  ...
  for var in fl_vars:
    ...
    if f_var in stmt_line:
      if "dd_real" in stmt_line or "double" in stmt_line:
        ret = 4
  if ret == 0 and "double" in stmt_line:
    if "=" in stmt_line and "double" in stmt_line.split("=")[0]:
      ret = 1
    elif "=" not in stmt_line and "double" in stmt_line:
      ret = 1
...
if dyblock_enabled == 1:
  with open(ben_dir+"block_list.txt", "r") as var_f:
    for line in var_f:
      line = line.replace("\n", "").replace(" ", "");
      block_list.append(line)
  
  print "block: " + str(block_list)
...
if dyblock_enabled == 1:
  subprocess.call(['./Translator ' + '-DRD_WG_SIZE_0_0=' + block_list[0] + ' ' + str(sys.argv[1])], shell=True)
else:
    ...
    if is_global(dv_glb, stmt_line) and "cuda" not in stmt_line:
      if "double" in stmt_line and "=" not in stmt_line and "(double)" not in stmt_line:
        if "constant" not in stmt_line:
          print(stmt_line.replace('double', '__constant__ gdd_real'))
          continue
        else:
...
if dyblock_enabled == 1:
  subprocess.call(['cd ' + ' ./cuda_bench/hotspot; make clean;' + 'make C_FLAG+=-DRD_WG_SIZE_0_0=' + block_list[0] + ';./hotspot_dd 1024 4 1000 ../data/hotspot/temp_1024 ../data/hotspot/power_1024'], shell=True)
else:
  subprocess.call(['cd ' + ' ./cuda_bench/hotspot; make clean;make; ./hotspot_dd 1024 4 1000 ../data/hotspot/temp_1024 ../data/hotspot/power_1024'], shell=True)
#exit()
#exit()
print "\ncalling func:\n"
...
#initialize counter for block_list offset
bl_off = 1
...
for x in range(0, tune_round):
  for func in comp_vars:
    ...
    for cur_var in comp_vars[func]:
          ...
          for stmt in stmt_list:
            ...
            if cur_var in global_var:
              if "gdd_real" in stmt and "cuda" not in stmt and "dealloc" not in stmt  and "upload" not in stmt and "alloc" not in stmt and if_var_in_stmt(cur_var, stmt)==True:
                ...
                dv_def = stmt
            # param function 
            if "(" in stmt and ")" in stmt and ";" not in stmt and "if" not in stmt and "else" not in stmt and "while" not in stmt and "switch" not in stmt:
              inside_func = True
              del dv_func[:]
              #clear the func_call 
              if "{" in stmt:
      ...
      if cur_var == "MatrixTemp" and is_single==False:
        is_single = True
      #elif cur_var == "finalVec" and is_single==True:
      elif cur_var == "MatrixTemp" and is_single==True:
        is_single = False
      #if cur_var == "MatrixPower" and is_single:
      #if cur_var == "MatrixTemp":
      is_run = True
      if is_run == True:
        ...
        with open("run_program.run", "a") as wline:
          for stmt in run_program:
            ...
            if "gettimeofday(&end_t" in stmt:
              ...
              elif cur_var == "MatrixTemp" and is_single==False:
      ...
      #  #run the translator for the original program
        if is_skip_for_var_combo==False:
          if dyblock_enabled == 1:
            subprocess.call(['cp ' + "run_program.run" + ' ./cuda_bench/hotspot/hotspot_dd.cu;' + ' cd ' + ' ./cuda_bench/hotspot; make clean;'+ 'make C_FLAG+=-DRD_WG_SIZE_0_0=' + block_list[bl_off] + '; ./hotspot_dd 1024 4 1000 ../data/hotspot/temp_1024 ../data/hotspot/power_1024 '], shell=True)
            bl_off += 1
          else:
            subprocess.call(['cp ' + "run_program.run" + ' ./cuda_bench/hotspot/hotspot_dd.cu;' + ' cd ' + ' ./cuda_bench/hotspot; make clean;make; ./hotspot_dd 1024 4 1000 ../data/hotspot/temp_1024 ../data/hotspot/power_1024 '], shell=True)
