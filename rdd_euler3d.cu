...
inline __device__ __host__ void compute_flux_contribution(gdd_real &density,gdd_real3 &momentum,gdd_real &density_energy,gdd_real &pressure,gdd_real3 &velocity,gdd_real3 &fc_momentum_x,gdd_real3 &fc_momentum_y,gdd_real3 &fc_momentum_z,gdd_real3 &fc_density_energy)
{
...
// read in domain geometry
  ...
  gdd_real *h_areas = new double [nelr];
  int *h_elements_surrounding_elements = new int [nelr * 4];
  gdd_real *h_normals = new double [nelr * 3 * 4];
  dd_real input;
// read in data
  for (int i = 0; i < nel; i++) {
    file >> input;
...
// Create arrays and set initial conditions
  gdd_real *variables = alloc< double  > (nelr * (1 + 3 + 1));
  initialize_variables(nelr,variables);
  gdd_real *old_variables = alloc< double  > (nelr * (1 + 3 + 1));
  gdd_real *fluxes = alloc< double  > (nelr * (1 + 3 + 1));
  gdd_real *step_factors = alloc< double  > (nelr);
