...
void ForwardSub();
void BackSub();
__global__ void Fan1(double *m, double *a, int Size, int t);
__global__ void Fan2(double *m, double *a, double *b,int Size, int j1, int t);
...
void BackSub()
{
  ...
  mpf_t val_x;
  mpf_init2(val_x, 128);
  FILE *outfile = fopen("fv_ref.txt", "w");
