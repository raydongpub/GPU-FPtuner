...

{
 ...
 check ( ( cudaMalloc ( ( void * * ) & t, sizeof ( T ) * N ) ), "cudaMalloc((void**)&t, sizeof(T)*N)", "euler3d_double.cu", 86 );
...

{
 ...
 check ( ( cudaFree ( ( void * ) array ) ), "cudaFree((void*)array)", "euler3d_double.cu", 93 );
...

{
 ...
 check ( ( cudaMemcpy ( ( void * ) dst, ( void * ) src, N * sizeof ( T ), cudaMemcpyDeviceToDevice ) ), "cudaMemcpy((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToDevice)", "euler3d_double.cu", 99 );
...

{
 ...
 check ( ( cudaMemcpy ( ( void * ) dst, ( void * ) src, N * sizeof ( T ), cudaMemcpyHostToDevice ) ), "cudaMemcpy((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyHostToDevice)", "euler3d_double.cu", 105 );
...

{
 ...
 check ( ( cudaMemcpy ( ( void * ) dst, ( void * ) src, N * sizeof ( T ), cudaMemcpyDeviceToHost ) ), "cudaMemcpy((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToHost)", "euler3d_double.cu", 111 );
...
void compute_flux_contribution_host(dd_real &density,dd_real3 &momentum,dd_real &density_energy,dd_real &pressure,dd_real3 &velocity,dd_real3 &fc_momentum_x,dd_real3 &fc_momentum_y,dd_real3 &fc_momentum_z,dd_real3 &fc_density_energy)
...
inline __device__ void compute_velocity_1(gdd_real &density,gdd_real3 &momentum,gdd_real3 &velocity)
...
inline __device__ void compute_velocity_2(gdd_real &density,gdd_real3 &momentum,gdd_real3 &velocity)
...
inline __device__ gdd_real compute_speed_sqd_1(gdd_real3 &velocity)
...
inline __device__ gdd_real compute_speed_sqd_2(gdd_real3 &velocity)
...
inline __device__ double compute_pressure(gdd_real &density,gdd_real &density_energy,gdd_real &speed_sqd)

{

  return to_double((((double )1.4) - ((double )1.0)) * (density_energy - ((double )0.5) * density * speed_sqd));

}

inline __device__ gdd_real compute_pressure_1(gdd_real &density,gdd_real &density_energy,gdd_real &speed_sqd)
...
inline __device__ gdd_real compute_pressure_2(gdd_real &density,gdd_real &density_energy,gdd_real &speed_sqd)
...
inline __device__ gdd_real compute_speed_of_sound(gdd_real &density,double &pressure)
...
inline __device__ gdd_real compute_speed_of_sound_1(gdd_real &density,gdd_real &pressure)
...
inline __device__ gdd_real compute_speed_of_sound_2(gdd_real &density,gdd_real &pressure)
...

{
  ...
  ::gdd_real speed_sqd_i = compute_speed_sqd_1(velocity_i);

  ::gdd_real speed_i = sqrt(speed_sqd_i);

  ::gdd_real pressure_i = compute_pressure_1(density_i,density_energy_i,speed_sqd_i);

  ::gdd_real speed_of_sound_i = compute_speed_of_sound_1(density_i,pressure_i);
...
// CUDA_SAFE_CALL(cudaGetDeviceProperties(&prop, dev));
  ...
  check((cudaSetDevice(0)),"cudaSetDevice(0)","euler3d_double.cu",471);

  check((cudaGetDevice(&dev)),"cudaGetDevice(&dev)","euler3d_double.cu",472);

  check((cudaGetDeviceProperties(&prop,dev)),"cudaGetDeviceProperties(&prop, dev)","euler3d_double.cu",473);
...
// copy far field conditions to the gpu
  ...
  gdd_real nh_ff_variable[5];

qd2gqd(h_ff_variable,nh_ff_variable,5);

  check((cudaMemcpyToSymbol(ff_variable,nh_ff_variable,(1 + 3 + 1) * sizeof(gdd_real ))),"cudaMemcpyToSymbol(ff_variable, nh_ff_variable, NVAR*sizeof(gdd_real))","euler3d_gdd_real.cu",510);

  gdd_real3 nh_ff_flux_contribution_momentum_x;

qd2gqd(&h_ff_flux_contribution_momentum_x,&nh_ff_flux_contribution_momentum_x,1);

  check((cudaMemcpyToSymbol(ff_flux_contribution_momentum_x,(&nh_ff_flux_contribution_momentum_x),sizeof(::gdd_real3 ))),"cudaMemcpyToSymbol(ff_flux_contribution_momentum_x, &nh_ff_flux_contribution_momentum_x, sizeof(gdd_real3))","euler3d_gdd_real.cu",511);

  gdd_real3 nh_ff_flux_contribution_momentum_y;

qd2gqd(&h_ff_flux_contribution_momentum_y,&nh_ff_flux_contribution_momentum_y,1);

  check((cudaMemcpyToSymbol(ff_flux_contribution_momentum_y,(&nh_ff_flux_contribution_momentum_y),sizeof(::gdd_real3 ))),"cudaMemcpyToSymbol(ff_flux_contribution_momentum_y, &nh_ff_flux_contribution_momentum_y, sizeof(gdd_real3))","euler3d_gdd_real.cu",512);

  gdd_real3 nh_ff_flux_contribution_momentum_z;

qd2gqd(&h_ff_flux_contribution_momentum_z,&nh_ff_flux_contribution_momentum_z,1);

  check((cudaMemcpyToSymbol(ff_flux_contribution_momentum_z,(&nh_ff_flux_contribution_momentum_z),sizeof(::gdd_real3 ))),"cudaMemcpyToSymbol(ff_flux_contribution_momentum_z, &nh_ff_flux_contribution_momentum_z, sizeof(gdd_real3))","euler3d_gdd_real.cu",513);

  gdd_real3 nh_ff_flux_contribution_density_energy;

qd2gqd(&h_ff_flux_contribution_density_energy,&nh_ff_flux_contribution_density_energy,1);

  check((cudaMemcpyToSymbol(ff_flux_contribution_density_energy,(&nh_ff_flux_contribution_density_energy),sizeof(::gdd_real3 ))),"cudaMemcpyToSymbol(ff_flux_contribution_density_energy, &nh_ff_flux_contribution_density_energy, sizeof(gdd_real3))","euler3d_gdd_real.cu",515);
...
// duplicate the last element
  ...
  gdd_real *nh_areas = new gdd_real [nelr];

qd2gqd(h_areas,nh_areas,nelr);

  upload< gdd_real  > (areas,nh_areas,nelr);
  ...
  gdd_real *nh_normals = new gdd_real [nelr * 3 * 4];

qd2gqd(h_normals,nh_normals,nelr * 3 * 4);

  upload< gdd_real  > (normals,nh_normals,nelr * 3 * 4);
