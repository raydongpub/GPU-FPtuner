...
int main(int argc, char** argv)
{
  ...
  FilesavingTemp = new double[size];
  FilesavingPower =  new double[size];
  MatrixOut = new double[size];
  for (int i=0; i<size; i++)
    MatrixOut[i] = (double)0.0;
  //memset(MatrixOut, 0, size*sizeof(double));
  ...
  int src = 1, dst = 0;
	...
	for (ts = 0; ts < total_iterations; ts+=pyramid_height) {
     int temp = src;
     src = dst;
     dst = temp;
     calculate_temp<<<dimGrid, dimBlock>>>(MIN(pyramid_height, total_iterations-ts), MatrixPower,MatrixTemp[src],MatrixTemp[dst],\
  ...
  std::cout << "time: " << ((time_end.tv_sec + time_end.tv_usec*1e-6) - (time_start.tv_sec + time_start.tv_usec*1e-6)) << "\n";
