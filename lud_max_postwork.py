...
if dyblock_enabled == 1:
  subprocess.call(['cd ' + ' ./cuda_bench/lud; make clean;' + 'make C_FLAG+=-DRD_WG_SIZE_0_0=' + block_list[0] + '; ./lud_dd -s 8192'], shell=True)
else:
  subprocess.call(['cd ' + ' ./cuda_bench/lud; make clean;make; ./lud_dd -s 8192'], shell=True)
    ...
    for cur_var in comp_vars[func]:
      ...
      #  #run the translator for the original program
        if is_skip_for_var_combo==False:
          if dyblock_enabled == 1:
            subprocess.call(['cp ' + "run_program.run" + ' ./cuda_bench/lud/lud_dd.cu;' + ' cd ' + ' ./cuda_bench/lud; make clean;' + 'make C_FLAG+=-DRD_WG_SIZE_0_0=' + block_list[bl_off] + '; ./lud_dd -s 8192'], shell=True)
            bl_off += 1
          else:
            subprocess.call(['cp ' + "run_program.run" + ' ./cuda_bench/lud/lud_dd.cu;' + ' cd ' + ' ./cuda_bench/lud; make clean;make; ./lud_dd -s 8192'], shell=True)
