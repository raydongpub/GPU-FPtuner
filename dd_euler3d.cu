...
inline __device__ __host__ void compute_flux_contribution(double &density,::double3 &momentum,double &density_energy,double &pressure,::double3 &velocity,::double3 &fc_momentum_x,::double3 &fc_momentum_y,::double3 &fc_momentum_z,::double3 &fc_density_energy)
{
...
inline __device__ void compute_velocity(double &density,::double3 &momentum,::double3 &velocity)
{
...
inline __device__ double compute_speed_sqd(::double3 &velocity)
{
...
inline __device__ double compute_pressure(double &density,double &density_energy,double &speed_sqd)
{
...
inline __device__ double compute_speed_of_sound(double &density,double &pressure)
{
...
#pragma function 9 2
  ...
  ::double3 flux_contribution_i_momentum_y;
  ::double3 flux_contribution_i_momentum_z;
  ...
  ::double3 flux_contribution_nb_momentum_y;
  ::double3 flux_contribution_nb_momentum_z;
...
#pragma function 5 2
  const int i = (blockDim . x * blockIdx . x + threadIdx . x);
  double factor = step_factors[i] / ((double )(3 + 1 - j));
...
// CUDA_SAFE_CALL(cudaGetDeviceProperties(&prop, dev));
  check((cudaSetDevice(0)),"cudaSetDevice(0)","euler3d_double.cu",446);
  check((cudaGetDevice(&dev)),"cudaGetDevice(&dev)","euler3d_double.cu",447);
  check((cudaGetDeviceProperties(&prop,dev)),"cudaGetDeviceProperties(&prop, dev)","euler3d_double.cu",448);
  printf("Name:                     %s\n",prop . name);
// set far field conditions and load them into constant memory on the gpu
  double h_ff_variable[5];
  ...
  double ff_speed_of_sound = sqrt(1.4 * ff_pressure / h_ff_variable[0]);
  double ff_speed = ((double )1.2) * ff_speed_of_sound;
  ::double3 ff_velocity;
...
// copy far field conditions to the gpu
  check((cudaMemcpyToSymbol(ff_variable,h_ff_variable,(1 + 3 + 1) * sizeof(double ))),"cudaMemcpyToSymbol(ff_variable, h_ff_variable, NVAR*sizeof(double))","euler3d_double.cu",485);
  check((cudaMemcpyToSymbol(ff_flux_contribution_momentum_x,(&h_ff_flux_contribution_momentum_x),sizeof(::double3 ))),"cudaMemcpyToSymbol(ff_flux_contribution_momentum_x, &h_ff_flux_contribution_momentum_x, sizeof(double3))","euler3d_double.cu",486);
  check((cudaMemcpyToSymbol(ff_flux_contribution_momentum_y,(&h_ff_flux_contribution_momentum_y),sizeof(::double3 ))),"cudaMemcpyToSymbol(ff_flux_contribution_momentum_y, &h_ff_flux_contribution_momentum_y, sizeof(double3))","euler3d_double.cu",487);
  check((cudaMemcpyToSymbol(ff_flux_contribution_momentum_z,(&h_ff_flux_contribution_momentum_z),sizeof(::double3 ))),"cudaMemcpyToSymbol(ff_flux_contribution_momentum_z, &h_ff_flux_contribution_momentum_z, sizeof(double3))","euler3d_double.cu",488);
  check((cudaMemcpyToSymbol(ff_flux_contribution_density_energy,(&h_ff_flux_contribution_density_energy),sizeof(::double3 ))),"cudaMemcpyToSymbol(ff_flux_contribution_density_energy, &h_ff_flux_contribution_density_energy, sizeof(double3))","euler3d_double.cu",490);
  int nel;
  int nelr;
// read in domain geometry
  ...
  double *h_normals = new double [nelr * 3 * 4];
  double input;
// read in data
  for (int i = 0; i < nel; i++) {
    file >> input;
...
// duplicate the last element
  ...
  normals = alloc< double  > (nelr * 3 * 4);
  upload< double  > (normals,h_normals,nelr * 3 * 4);
  delete []h_areas;
  delete []h_elements_surrounding_elements;
  delete []h_normals;
// Create arrays and set initial conditions
  double *variables = alloc< double  > (nelr * (1 + 3 + 1));
  initialize_variables(nelr,variables);
  double *old_variables = alloc< double  > (nelr * (1 + 3 + 1));
  double *fluxes = alloc< double  > (nelr * (1 + 3 + 1));
...
// Begin iterations
  for (int i = 0; i < 2000; i++) {
    copy< double  > (old_variables,variables,nelr * (1 + 3 + 1));
