...
__device__ inline void compute_flux_contribution(double& density, double3& momentum, double& dens_energy, double& pressure, double3& velocity, double3& fc_momentum_x, double3& fc_momentum_y, double3& fc_momentum_z, double3& fc_density_energy)
{
	...
	double de_p = dens_energy+pressure;
	fc_density_energy.x = velocity.x*de_p;
	fc_density_energy.y = velocity.y*de_p;
	fc_density_energy.z = velocity.z*de_p;
}

void compute_flux_contribution_host(double& density, double3& momentum, double& dens_energy, double& pressure, double3& velocity, double3& fc_momentum_x, double3& fc_momentum_y, double3& fc_momentum_z, double3& fc_density_energy)
{
	fc_momentum_x.x = velocity.x*momentum.x + pressure;
	fc_momentum_x.y = velocity.x*momentum.y;
	fc_momentum_x.z = velocity.x*momentum.z;
...
__global__ void cuda_compute_flux(int nelr, int* elements_surrounding_elements, double* normals, double* variables, double* fluxes)
{
	...
	int j, nb;
	double3 normal; double norm_len;
	double factor;
	...
	for(j = 0; j < NNB; j++)
	{
		...
		if(nb >= 0) 	// a legitimate neighbor
		{
			...
			factor = -norm_len*smoothing_coefficient*double(0.5)*(speed_i + sqrt(speed_sqd_nb) + speed_of_sound_i + speed_of_sound_nb);
...
int main(int argc, char** argv)
{
  ...
  struct timeval ske_t;
	...
	// set far field conditions and load them into constant memory on the gpu
		...
		compute_flux_contribution_host(h_ff_variable[VAR_DENSITY], h_ff_momentum, h_ff_variable[VAR_DENSITY_ENERGY], ff_pressure, ff_velocity, h_ff_flux_contribution_momentum_x, h_ff_flux_contribution_momentum_y, h_ff_flux_contribution_momentum_z, h_ff_flux_contribution_density_energy);
	...
	double* normals;
      ...
      h_areas[i] = input;
        ...
        h_elements_surrounding_elements[i + j*nelr] = input;
				...
				{
					file >> input;
          h_normals[i + (j + k*NNB)*nelr] = input;
