...
def max_pattern(var_list, host_vars, db_vars, h_new, n_size, n_ptr, stmt_line):
  ret = 0
  #check device variable list
  for var in var_list:
    stmt_var = var.replace("\n", "")
    if var in stmt_line:
      if "double" in stmt_line:
        if "=" in stmt_line and "double" in stmt_line.split("=")[0] and "var" in stmt_line.split("=")[0]:
...
def get_host_dv(cur_v, func, dv_host):
  host_gdd = None
  for stmt in func:
    if ("cudaMemcpy" in stmt or "upload" in stmt)  and if_var(cur_v, stmt):
      host_gdd = get_var(cur_v, stmt, 1).replace("\n", "")
      dv_host.append(host_gdd.replace("\n", ""))
      #dv_host.append(get_var(cur_v, stmt, 1))
  for stmt in func:
    if "qd2gqd" in stmt and if_var(host_gdd, stmt):
      dv_host.append(get_var(host_gdd, stmt, -1).replace("\n", ""))
...
def change_type(cur_v, stmt):
  if "=" in stmt and cur_v in stmt.split('=')[0]:
    ...
    if "::dd_real" in stmt:
      ...
      stmt = stmt.replace("dd_real", "double")
      return stmt
    elif "double" in stmt and "(double)" not in stmt.split('=')[0] and "(double )" not in stmt.split('=')[0]:
  ...
  for index, stmt in enumerate(func): 
    ...
    if len(hd_var) == 2:
      if if_var(h_gdd, stmt)==True and "cuda" not in stmt and "upload" not in stmt:
        ...
        continue
      if if_var(h_gdd, stmt) and "qd2gqd" in stmt:
        stmt_remove.append(stmt)
        continue
...
with closing(fileinput.FileInput(sys.argv[2], inplace=True, backup='.bak')) as stmt_list:
  ...
  for stmt_line in stmt_list:
    ...
    if is_gdd(host_vars, stmt_line): 
      ...
      ret = max_pattern(dev_vars, host_vars, db_vars, host_new, new_size, new_ptr, stmt_line) 
      if ret == 1:
        print(stmt_line.replace('double', 'gdd_real'))
      elif ret == 11:
        counter += 1
        ...
        print(stmt_line.replace('dd_real', 'double'))
      #if f_cnt == 0:
      #  visit = 0
...
subprocess.call(['cd ' + ' ./cuda_bench/sp; make clean;make; ./sp_dd b'], shell=True)
#exit()
print "\ncalling func:\n"
...
for x in range(0, tune_round):
  for func in comp_vars:
    ...
    for cur_var in comp_vars[func]:
          ...
          for stmt in stmt_list:
            ...
            if cur_var in global_var:
              if "gdd_real" in stmt and "cuda" not in stmt and "dealloc" not in stmt  and "upload" not in stmt and "alloc" not in stmt and if_var_in_stmt(cur_var, stmt)==True:
                ...
                dv_def = stmt
            # param function 
            if "(" in stmt and ")" in stmt and ";" not in stmt and "if" not in stmt and "else" not in stmt and "while" not in stmt:
              inside_func = True
              del dv_func[:]
              #clear the func_call 
              if "{" in stmt:
      ...
      if cur_var == "fu" and is_single==False:
        is_single = True
      elif cur_var == "fu" and is_single==True:
        is_single = False
      #if cur_var == "eta":
      #if cur_var == "fu" and func== "add_kernel":
      is_run = True
      if is_run == True:
            ...
            if "gettimeofday(&end_t" in stmt:
              ...
              if is_single:
                for line in db_cmp_insert:
                  wline.write(line+"\n")
              elif cur_var == "fu" and is_single==False:
      ...
      #  #run the translator for the original program
        if is_skip_for_var_combo==False:
          subprocess.call(['cp ' + "run_program.run" + ' ./cuda_bench/sp/sp_dd.cu;' + ' cd ' + ' ./cuda_bench/sp; make clean;make; ./sp_dd b'], shell=True)
