  ...
  for var in host_vars: 
    ...
    if if_var_decl(h_var, stmt_line) and "main" not in stmt_line:
      if ( ("cudaMemcpy" in stmt_line or "upload" in stmt_line) and ret == 1):
        if (h_var in h_new):
          if "HostToDevice" in stmt_line or "upload" in stmt_line:
              nh_var = "n"+h_var
              #if h_var == "hm":
...
def get_host_dv(cur_v, func, dv_host):
  host_gdd = None
  for stmt in func:
    if (("cudaMemcpy" in stmt and "HostToDevice" in stmt) or "upload" in stmt)  and if_var(cur_v, stmt):
...
subprocess.call(['cd ' + ' ./cuda_bench/gaussian; make clean;make; ./gaussian_dd -s 8192'], shell=True)
#exit()
print "\ncalling func:\n"
  ...
  for func in comp_vars:
    ...
    for cur_var in comp_vars[func]:
      ...
      if cur_var == "finalVec" and is_single==False:
        ...
        is_single = False
      #if cur_var == "rstmp" and func== "add_kernel" and is_single:
      ...
      #  #run the translator for the original program
        if is_skip_for_var_combo==False:
          subprocess.call(['cp ' + "run_program.run" + ' ./cuda_bench/gaussian/gaussian_dd.cu;' + ' cd ' + ' ./cuda_bench/gaussian; make clean;make; ./gaussian_dd -s 8192'], shell=True)
